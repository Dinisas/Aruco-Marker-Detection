...
class EnhancedArucoDetector:
    ...
    def fit_range_error_model(self):
        ...
        try:
            ...
            popt, pcov = optimize.curve_fit(linear_model, distances, stds, p0=p0)
            alpha, beta = popt
            
            # Calculate goodness of fit
            predicted_stds = linear_model(np.array(distances), alpha, beta)
    ...
    def process_calibration_results(self):
        ...
        if self.range_measurements:
            ...
            if range_params:
                ...
                print(f"α = {range_params['alpha']:.6f} (linear coefficient)")
                ...
                for d in test_distances:
                    sigma_r = range_params['alpha'] * d + range_params['beta']
    ...
    def save_calibration_results(self, range_params, bearing_params):
        ...
        filename = f'aruco_calibration_linear_{self.image_width}x{self.image_height}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
    ...
    def generate_usage_code(self, range_params, bearing_params):
        ...
        if range_params:
            ...
            code += f"    sigma_r = {range_params['alpha']:.6f} * distance_meters + {range_params['beta']:.6f}\n"
    ...
    def plot_calibration_results(self, range_params, bearing_params):
        ...
        if range_params and self.range_measurements:
            ...
            sigma_fit = range_params['alpha'] * d_fit + range_params['beta']
        ...
        plt.tight_layout()
        filename = f'calibration_plots_linear_{datetime.now().strftime("%Y%m%d_%H%M%S")}.png'
...
if __name__ == "__main__":
    ...
    detector = EnhancedArucoDetector(
        ...
        marker_length=0.16,  # 16 cm markers
        camera_matrix=camera_matrix,
        dist_coeffs=dist_coeffs
    )
    
    print("\nEnhanced ArUco Detector - LINEAR MODEL")
    print("=====================================")
    print("Range Model: σ_r(d) = α·d + β")
    print("Bearing Model: σ_θ = α + β/s")
    print("\nCommands:")
    print("- Normal detection: Just run the program")
    print("- Calibration mode: Press 'c' during detection")
